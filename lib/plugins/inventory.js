...
function inject (bot, { version, hideErrors }) {
  ...
  function extendWindow (window) {
    ...
    window.withdrawWithOptions = async (options) => {
      options.sourceStart = 0
      options.sourceEnd = window.inventoryStart
      options.destStart = window.inventoryStart
      options.destEnd = window.inventoryEnd
      options.window = window
      options.count = options.count || 1

      if (options.matcher && typeof options.matcher === 'function') {
        const item = window.slots.filter((_, i) => i >= options.sourceStart && i < options.sourceEnd).find(options.matcher)
        if (!item) throw Error('No item found by matcher function')
        options.itemType = item.type
        options.metadata = item.metadata
        options.nbt = item.nbt
      }

      return transfer(options)
    }
    window.depositWithOptions = async (options) => {
      options.sourceStart = window.inventoryStart
      options.sourceEnd = window.inventoryEnd
      options.destStart = 0
      options.destEnd = window.inventoryStart
      options.window = window
      options.count = options.count || 1
