...
function inject (bot, { version, hideErrors }) {
  ...
  let lastWindowClick = Date.now()

  async function waitForClickDelay (amount) {
    if (lastWindowClick + amount < Date.now()) return Promise.resolve()
    await wait(lastWindowClick + amount - Date.now())
  }
  ...
  async function putSelectedItemRange (start, end, window, slot, noWaiting = false, waitBetweenClicks = null) {
    ...
    while (window.selectedItem) {
      ...
      if (item && item.stackSize !== item.count) { // something to join with
        if (noWaiting) {
          if (waitBetweenClicks) {
            await waitForClickDelay(waitBetweenClicks)
            lastWindowClick = Date.now()
          }
          await clickWindow(item.slot, 0, 0)
        } else {
          if (waitBetweenClicks) {
            await Promise.all([clickWindow(item.slot, 0, 0), once(window, `updateSlot:${item.slot}`), wait(lastWindowClick + waitBetweenClicks - Date.now()), waitForClickDelay(waitBetweenClicks)])
            lastWindowClick = Date.now()
          } else {
            await Promise.all([clickWindow(item.slot, 0, 0), once(window, `updateSlot:${item.slot}`), wait(lastWindowClick + waitBetweenClicks - Date.now())])
            lastWindowClick = Date.now()
          }
        }
        ...
        if (emptySlot === null) { // no room left
          ...
          if (noWaiting) {
            if (waitBetweenClicks) {
              await Promise.all([clickWindow(emptySlot, 0, 0), once(window, `updateSlot:${emptySlot}`), waitForClickDelay(waitBetweenClicks)])
              lastWindowClick = Date.now()
            } else {
  ...
  async function transfer (options) {
    ...
    const waitBetweenClicks = options.waitBetweenClicks || null
    const noWaiting = options.noWaiting || null
    ...
    async function transferOne () {
      if (count === 0) {
        await putSelectedItemRange(sourceStart, sourceEnd, window, firstSourceSlot, noWaiting, waitBetweenClicks)
        return
      }
  ...
  function extendWindow (window) {
    ...
    window.withdraw = callbackify(async (itemType, metadata, count, waitBetweenClicks = null) => {
    ...
    window.deposit = callbackify(async (itemType, metadata, count, waitBetweenClicks = null) => {
      const options = {
        window: window,
        itemType,
        metadata,
...
=======
  async function putAway (slot, noWaiting = false, waitBetweenClicks = null) {
>>>>>>> d48993f (waitBetweenClicks option for inventory functions)
    const window = bot.currentWindow || bot.inventory
...
=======
    await putSelectedItemRange(start, end, window, null, noWaiting, waitBetweenClicks)
>>>>>>> d48993f (waitBetweenClicks option for inventory functions)
    await promisePutAway
  }

  async function moveSlotItem (sourceSlot, destSlot, waitBetweenClicks = null) {
    if (waitBetweenClicks) {
      await waitForClickDelay(waitBetweenClicks)
      lastWindowClick = Date.now()
    }
